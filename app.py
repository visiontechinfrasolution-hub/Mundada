...
if page == "🏠 Dashboard":
    ...
    except: st.info("Loading Dashboard...")
...
# --- 7. SITE DATA ENTRY (CLEAN SINGLE TABLE + UPLOAD) ---
    ...
    # ACTION BAR (UPLOAD/DOWNLOAD/NEW)
    ...
    search = tc4.text_input("🔍 Search Database...", placeholder="Search Site ID, Project ID, Status...")
    ...
    st.divider()
    # SINGLE CLEAN TABLE WITH HORIZONTAL SCROLL (ALL COLUMNS)
    if not df.empty:
        if search: df = df[df.astype(str).apply(lambda x: x.str.contains(search, case=False)).any(axis=1)]
        
        st.subheader("📋 Live Database")
        # Displaying edit buttons in one column, then data
        for idx, row in df.iterrows():
            r = st.columns([0.6, 2, 2, 2, 2, 2])
            if r[0].button("📝", key=f"btn_{row['id']}"):
                st.session_state.edit_row_data = row.to_dict(); st.rerun()
            r[1].write(f"**ID:** {row['project_id']}"); r[2].write(f"**Site:** {row['site_id']}"); r[3].write(row['site_name']); r[4].write(row['team_name']); r[5].write(row['site_status'])
        
        st.divider()
        st.markdown("### 📊 Raw Data View (Horizontal Scroll)")
