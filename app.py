    ...
    if st.session_state.show_modal:
        with st.expander("📝 SITE ENTRY FORM", expanded=True):
            ...
            with st.form("modal_form"):
                ...
                p_amt = c5.number_input("Project Amt", value=float(er['project_amt']) if er else 0.0)
                status = c6.selectbox("Status", ["Planning", "WIP", "WCC Done", "Closed"])

                c7, c8, c9 = st.columns(3)
                po_n = c7.text_input("PO No", value=er['po_no'] if er else "")
                po_a = c8.number_input("PO Amt", value=float(er['po_amt']) if er else 0.0)
                t_name = c9.text_input("Team Name", value=er['team_name'] if er else "")

                c10, c11, c12 = st.columns(3)
                t_bill = c10.number_input("Team Billing", value=float(er['team_billing']) if er else 0.0)
                t_paid = c11.number_input("Team Paid", value=float(er['team_paid_amt']) if er else 0.0)
                wcc_n = c12.text_input("WCC No", value=er['wcc_no'] if er else "")

                c13, c14, c15 = st.columns(3)
                wcc_a = c13.number_input("WCC Amt", value=float(er['wcc_amt']) if er else 0.0)
